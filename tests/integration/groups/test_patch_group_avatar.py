...
async def test_patch_group_avatar_success_full_access(client: AsyncClient, cdn_client: AsyncClient):
    user = await register_and_login(client)
...
async def test_patch_group_avatar_success_manage_group(client: AsyncClient, cdn_client: AsyncClient):
    user = await register_and_login(client)
    user2 = await register_and_login(client)
    await set_authorization(client, user)
