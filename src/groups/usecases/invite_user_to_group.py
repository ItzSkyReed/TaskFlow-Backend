...
from ..exceptions import NotEnoughPermissionsException, GroupNotFoundByIdException
from ..models import GroupInvitation, InvitationStatus, GroupPermission, Group
...
async def invite_user_to_group(
    ...
    stmt = insert(GroupInvitation).values(
        group_id=group_id,
    ...
    invitation = (await session.execute(select(GroupInvitation).where(
        GroupInvitation.group_id == group_id,
        GroupInvitation.invitee_id == invitee_id,
        GroupInvitation.status == InvitationStatus.PENDING))).scalar_one()
