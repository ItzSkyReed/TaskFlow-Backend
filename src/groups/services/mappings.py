...
from .group_service import get_groups_user_context, get_group_user_context, get_group_member_count
from ..models import Group, GroupInvitation
from ..schemas import GroupDetailSchema, GroupMemberSchema, GroupSummarySchema, GroupSearchSchema, GroupInvitationSchema
...
async def map_to_group_invitation_schema(
    ...
    return GroupInvitationSchema(
        ...
        group=await map_to_group_summary_schema(
            invitation.group, user_id, session
        ),
    )
...
async def map_to_group_invitation_schemas(
    ...
    groups_summary_schemas = await map_to_group_summary_schemas([invitation.group for invitation in invitations], user_id, session)
    return [GroupInvitationSchema(
        id=invitation.id,
        status=invitation.status,
        updated_at=invitation.updated_at,
    ...
    ) for invitation, group_summary_schema in zip(invitations, groups_summary_schemas)]